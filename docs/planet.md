...
Planet V3 在 V2 的多 Root/多 Relay 排序基础上，强制为每个 Root、Relay 发布稳定的服务 ID 和服务身份策略。Planet V4 在此基础上可发布 TLS Relay：每项绑定既有 Relay 的 `service_id`、数值 TCP 端点、SNI 与叶证书 DER 的 SHA-256 指纹。Planet V5 再加入控制器签名的 TURN 端点；短期设备凭据不进入 Planet，而只通过入网和成员刷新 HTTPS 响应下发。V3/V4/V5 缺少服务身份、服务 ID 不匹配、签名不完整、已撤销密钥、TURN 端点格式错误或轮换窗口无效，均失败关闭。
...
- 刷新前验证签名、有效期、控制器 URL、V3/V4/V5 服务身份、TLS Relay 证书指纹、TURN 端点约束以及版本、`issued_at` 与语义摘要的单调性。拒绝版本回退、`issued_at` 回退，或相同版本/时间却改变语义的清单。
