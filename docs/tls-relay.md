...
```text
UDP 直连 -> 已认证 UDP Relay -> Planet V4 TLS Relay
```

`RelayPolicy::Disabled` 禁止所有 Relay 回退；`RelayPolicy::Required` 跳过直连。对于 V3/V4 Relay，只有收到对应网络、服务身份和 nonce 的已验证注册确认后，UDP Relay 才会被视为健康；UDP 未健康而 TLS Relay 已完成指纹钉扎连接时，客户端才把加密帧写入 TLS 流。
